...
Since S8 the table also holds *session items*, one per browser tab a student
opens, under the learner's partition with sort key `session#<sessionId>`.
They carry `type: 'session'`, `startedAt`, `lastSeenAt`, `durationSec` and
`heartbeats`. The client posts a heartbeat on load, every 60 seconds while the
tab is visible and once more as it goes hidden; each heartbeat adds the gap
since the previous one, capped at 120 seconds, so `durationSec` is active time
and a tab left open over lunch adds nothing. Keeping sessions in this table
honours the events-to-DynamoDB rule without a second table to create by hand,
at the cost of one discipline: every reader of a learner's partition or of the
whole table must skip session items, or a session would land in the
dashboard's completion denominator as a lesson nobody can name.
`progressService.isSessionItem` is that filter, applied in
`getStudentProgress` (which every learner-facing screen reads through) and in
the research export. Only students record sessions, as with every other
progress signal.
...
| Client
| 30 tests, 6 files
| The display helpers, the publish rule, the `BadgeIcon` fallback logic and the `useTimeOnTask` and `useSessionHeartbeat`
hooks, run in jsdom with React Testing Library. Babel is configured inline in
...
| Reported
| `avgAttemptsPerLearner` per lesson (`totalAttempts` over `uniqueLearners`,
one decimal), in the instructor breakdown. Only a submit is an attempt, so
Run-button experiments do not inflate it. The per-submission transcript is in
the research export.

| Hint usage frequency
| Reported
| `avgHintsUsed` per lesson (depth) and `hintRevealRate` per lesson (reach:
the share of learners who revealed at least one hint). Each stored submission
also carries `hintsUsedAtSubmit`, so attempts before and after a hint can be
compared.

| Badge acquisition rate
| Reported, dated since S8
| Every award writes a dated entry to `users.badgeAwards[]` alongside the id
in `badges[]`. The admin KPIs report `badgesAwardedTotal` and
`badgesAwardedByWeek` over the same window as the weekly-active series, and
the profile shows the date under each earned badge. Awards made before S8
carry no timestamp: they still count on the learner's profile but are absent
from the dated series.

| Average session duration
| Reported as active session duration, heartbeat-based
| Session items in the progress table (see "The progress table"). The admin
KPIs report `sessions`, `avgSessionDurationSec` and
`medianSessionDurationSec` over the window; a session's duration is the sum
of heartbeat gaps capped at 120 seconds, so it is active time in the app, not
wall-clock time between first and last request. `timeSpent` per lesson
(`avgTimeSpentSec`) remains the per-lesson time-on-task figure.
|===
...
| The client has 30 tests over its display helpers, one component and the
time-on-task and session-heartbeat hooks. No page-level component is tested; the production build in
