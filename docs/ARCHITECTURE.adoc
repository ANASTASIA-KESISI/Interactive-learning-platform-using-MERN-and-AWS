...
| Login (live and local) | 97 | No `main` landmark
| Student signup | 98 | No `main` landmark
| Instructor signup | 98 | No `main` landmark
|===

The one failure is shared: the app shell renders content without a `<main>`
element, so assistive technology has no landmark to jump to. It is a
one-element fix in the layout.
...
The default redirect rule serves `index.html` for unknown paths, so deep links
render. Measured on 2026-09-13, though, the status codes are wrong: `/login`
answers 301 to `/login/`, and that answers 404 with the `index.html` body.
Browsers render it and users never notice, but crawlers, uptime probes and
Lighthouse read the status and treat the page as broken. The fallback needs to
be a `200` rewrite; which console setting is responsible has not been traced
beyond the symptom, and `DEPLOYMENT.md` has the check that proves it fixed.
...
As of 2026-09-13 that setup has been prepared but not run on the live
instance, so the API logs exist only in the host's journal; `DEPLOYMENT.md`
section 7 has the steps and the checklist item.
...
What is not there: no custom metrics, no alarms and no dashboards. The alarms
worth adding first are API 5xx rate, Lambda errors and throttles, and DynamoDB
throttled requests.
...
Application logs are structured JSON in CloudWatch. There are no alarms yet, so
an outage is noticed by a person rather than paged. Uptime for the pilot is
targeted at 99.5%.
...
| NFR5 | WCAG 2.1 AA | Designed toward AA and validated with automated tooling (Lighthouse 97–98 on the public pages); conformance not claimed — see gaps below
| NFR6 | 99.5% uptime during the pilot | systemd restart; CloudWatch logs, no alarms yet
|===
...
| Lighthouse scores the public pages 97–98 (2026-09-13) with one automated
failure, a missing `main` landmark. The authenticated screens have not been
...
| Deep links return a 404 status
| On Amplify, `/login` is a 301 to `/login/`, which is a 404 carrying the
`index.html` body. The page renders, so no user has noticed, but any client
that reads the status sees a broken page. Traced to the symptom only; see the
Amplify section.
...
| API logs not yet in CloudWatch
| The Lambda runners and Amplify log to CloudWatch automatically; the API does
not. Its structured logs sit in the instance's journal until
`deploy/setup-cloudwatch.sh` is run with `CloudWatchAgentServerPolicy` on the
instance role. Prepared, not applied, as of 2026-09-13.

| No CloudWatch alarms
| CloudWatch holds logs only. No alarms, dashboards or custom metrics are
configured, so a 5xx spike or a throttled Lambda is found by reading logs, not
by being told.
