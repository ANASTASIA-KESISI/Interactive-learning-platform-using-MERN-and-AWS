...
The full entity diagram — every collection, its fields, and the relationships
between them — is the *Data model* page of:

`docs/learncode-architecture.drawio`

Open it at https://app.diagrams.net[app.diagrams.net] (menu:File[Open from ▸
Device]) or with the *Draw.io Integration* extension in VS Code. Export to SVG
or PNG beside this file if you need it embedded in the thesis.
...
The *AWS topology* page of `docs/learncode-architecture.drawio` draws what
follows. Concrete resource identifiers live in `DEPLOYMENT.md`, deliberately in
...
| `docs/learncode-architecture.drawio` | Two-page editable diagram: the data model, and the AWS topology.
|===
